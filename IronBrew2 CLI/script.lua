...
-- Make sure these match the values in {{GenerateGunDataStatsAttributesTable}} script on the server
...
function BulletModule:IgnoreList(IgnorePlayers, CustomIgnoreList)
	...
	if (IgnorePlayers) then
		...
		-- Ignore all character root parts (not the normal parts)
...
--[[**
	Bullet simulation function. This is thread-blocking!
...
function BulletModule:BulletSimulation( Player, StartPosition, Direction, GunData, Tracer, CustomTracerFolder, dts, test_positions, TracerOffsetPosition )
	...
	if ( Tracer and not test_positions ) then
		...
		local scalar = 0.25 -- TODO one day figure out why I need to do it this way...
	...
	while ( true ) do
		...
		-- Mark DTs (For server reconstruction)
		step = step + 1;
		table.insert(new_dts, dt);

		-- Compute offset with gravity
		GSpeed = (Gravity * TravelTime ^ 2)

		-- Compute bullet speed factor
		BulletSpeed = BulletSpeed-- * (1 - (BallisticCoefficient * dt));

		-- Compute remaining max distance
		...
		-- Find next position affected by gravity
...
function _cleanup(Tracer, CustomTracer)
	if ( Tracer and Tracer:IsA("Instance") ) then
		...
		-- Test hit object
		game:GetService("Debris"):AddItem(Tracer, WaitTime); -- Give the trail time to finish
	end
...
--[[**
	Returns Tuple that expresses which character was hit, and what hit type.
--]]
...
function getCharacterFromObject( hitObject )
	-- Loop through parts until we find a player
	local tries = 0
...
Player.CharacterAdded:Connect(function(character)
	...
	Character = character
	--table.insert(CrosshairRaycastParams.FilterDescendantsInstances, Character)
...
--[[local GAME_KEY_OBJECT = Player:WaitForChild("RequiredString", 60*5);
if ( GAME_KEY_OBJECT ~= nil ) then
	GAME_KEY = GAME_KEY_OBJECT.Value;
	pcall(function() GAME_KEY_OBJECT:Destroy(); end);
end]]
...
function takeoutWeapon( weaponSlot, forceTakeout, IgnoreSafeTakeoutTime )
	...
	-- Delete animation saves. SUUUUUPER SLOWWWWW
	...
	-- Make sure it's not anchored/collide
	...
	-- Compute Aim Invisible parts
	debug.profilebegin("Find Aimparts")
	AimInvisibleParts = {}
	local children = gunViewModel:GetDescendants()
	for i = 1, #children do
		local v = children[i]
		--for _,v in pairs(gunViewModel:GetDescendants()) do
	...
	-- Tell server we're taking out gun
	ReplicatedStorage.GunEvent.GunCreate:FireServer( weaponSlot )

	-- Compute offset matrix (so the gun knows how to ADS)
	...
	-- Idle/Draw animation
	SetGunAnimation( "Idle", true ) -- Set first to idle
	SetGunAnimation( "Draw", true ) -- If there is a draw animation, it will get moved to this
...
function computeGunOffsetMatrix(CurrentWeaponData, ViewModel)
	...
	for _,v in cpairs(AttachmentData) do
		if ( v.Part ) then
			...
			--v.PartTemp:SetAttribute("OrientationOffset", Vector3.new(0, -math.pi/2, 0))
	...
	-- Why do we have to wait this long????? TODO Fix this terrible code...
...
function findAnimationReference( name )
	...
	-- Some animation types can have multiple names
...
function getNextAnimationLogic()
	...
	-- User-override for finished animation
	...
	-- Check if the animation ending is an action animation
	...
	-- Check if the animation ending is a movement animation
	local IsEndOfMovement = isBaseAnimation(currentlyPlayingAnimationName)

	-- Switch to rechamber if we need it
	--if ( (IsEndOfAction or IsEndOfMovement) and RequiresManualRechamber ) then
	--	rechamberGun()
	--	return
	--end
	...
	-- Return to idle after rechambering
...
function getAnimationPriority(animationObject)
	...
	if ( animationPriorityMap[animationName] ) then
		...
		-- Try to find any string matches
...
function SetGunAnimation(DesiredAnimationName, ForceAnimation, transition, forceSpeed, forceLooped)
	...
	--if ( not currentlyPlayingAnimation.Looped ) then
...
function onReloadGun()
	...
	--local ammo = InventoryItem.BulletsInMagazine.Value
	--local reserve = InventoryItem.BulletsInReserve.Value
	--local maxMag = gunCurrent.Stats.MagazineCapacity.Value
	--local subt = maxMag - ammo
	--if ( reloadingIndividual ) then
	--	subt = 1
	--end
	--if ( reserve - subt < 0 ) then
	--	subt = reserve
	--end

	--InventoryItem.BulletsInMagazine.Value = InventoryItem.BulletsInMagazine.Value + subt

	--local isMax = InventoryItem.BulletsInReserve.Value >= math.pow(2, 30)
	--if ( not isMax ) then
	--InventoryItem.BulletsInReserve.Value = InventoryItem.BulletsInReserve.Value - subt
	--end
	...
	--ReplicatedStorage.GunEvent.GunReload:FireServer(reloadingIndividual)
end
...
function getLookDirection()
	--silentAimTarget = workspace:WaitForChild("TestDummy")

	--if silentAimEnabled and silentAimTarget then
	--	local enemyPos = silentAimTarget.Head.Position
	--	local myPos = getCamera().CFrame.Position
	--	local distance = (enemyPos - myPos).Magnitude

	--	local currentSlot = getPlayer().CurrentSelectedObject.Value
	--	local gunData = currentSlot.Value
	--	local bulletSpeed = (gunData == nil or not gunData.Stats.BulletSettings:FindFirstChild("BulletSpeed")) and serverGunConfiguration:FindFirstChild("sv_default_bullet_speed").Value or gunData.Stats.BulletSettings.BulletSpeed.Value

	--	local timeToTarget = distance / bulletSpeed

	--	local movePrediction = Vector3.new(0, 0, 0)

	--	movePrediction = silentAimTarget.HumanoidRootPart.Velocity * timeToTarget * Testerz2

	--	local predictedPos = enemyPos + movePrediction

	--	local gunDataFolder = ReplicatedStorage:FindFirstChild("GunData")
	--	local gravity = serverGunConfiguration.sv_default_bullet_gravity.Value
...
function doPositionGun(delta, ViewModel)
	...
	-- Dont position if gun model isn't loaded in
	...
	-- Make sure view model has primary part
	--vmPrimaryPart.Anchored = true

	-- Calculate weapon run bob
	local div = 2
	local EnabledBob = script.ViewBobEnabled.Value
	bobticks = bobticks + (speed*(Jumping and 0 or 1))--*delta
	local TX = math.cos(bobticks / div)
	local TY = math.cos(bobticks / (div / 2))
	RUN_BOB_X = TX
	RUN_BOB_Y = TY

	-- Initial weapon bob vector
	...
	if ( (lastRunXPosition > 0 and TX < 0) or (lastRunXPosition < 0 and TX > 0) and not IsInAir() ) then
		--viewmodelTwitchSpring:Impulse(Vector3.new(
		--	(math.random()-math.random())*0.5,
		--	(math.random()-math.random()),
		--	(math.random()-math.random())
		--	)*RUN_BOB_SCALE*10*aimTween)
	end
	lastRunXPosition = TX

	-- Calculate weapon idle bob
	...
	idleticks = idleticks + SwaySteadyMultiplier*delta -- BREATH HOLDING
...
function updateCrosshair(Delta)
	...
	if ( crosshairGui ~= nil ) then
		...
		-- Position crosshair in center of screen (Though will be slightly offset in third person)
		...
		--local PartHit, WorldHit, NormalHit = workspace:FindPartOnRayWithIgnoreList(RayToPointFarAway, { workspace.Terrain, getCharacter(), getCamera(), workspace:FindFirstChild("IgnoreList") });
		...
		local scopeTrans = isScoped() and isFullyAimed() and 0.0 or 1.0 --math.max( 0, math.min( 1, (aimTween - 0.001) * 5 ) ) or 1
...
function getAttachments()
	...
	--for _,v in pairs(InventoryItem:GetChildren()) do
	for i = 1, #items do
		local v = items[i]
...
function disable()
	...
	--gunCurrent = getPlayer().GunInventory.Slot0.Value
end
...
function reloadLogic()
	local spd = tonumber(_G.GunModifiers["GunReloadSpeedMultiplier"]) or 1 -- reload speed
	...
	RepeatShoot = 0 -- Fix for burst fire overflow

	if ( hasAmmo and bulletsInGun < maxCapacity and not cancelReloadingIndividual ) then

		-- If we have no ammo left, rechamber when we have ammo			
		...
		if ( not reloadIndividual or not reloadIndividual.Value ) then
			...
			-- Reload single bullet (shotgun or some snipers, ect)
...
function checkSetRunning()
	...
	-- If we have a scoped gun (or even if we dont?) uncrouch and unaim if we sprint
	if ( gunCurrent ) then
...
function onKeyPress( inputObject, isProcessed )
	...
	-- Commands that we need keyboard control for
...
function onDeath()
	...
	-- This is a DUMB hack to force humanoid physics and make it understand that it died.
	-- No idea why this has to be done... Roblox humanoids are garbage.
	task.spawn(function()
		wait(0.1)
...
function onRespawn(Character)
	...
	Humanoid.HealthChanged:Connect(function(Health) -- This is a weird workaround... Died is NOT firing???????
		if ( Health <= 0 ) then
			IsDead = true
			CurrentlyInvisible = false
	...
	--getCustomCamera().IsEnabled.Value = true
	...
	-- Jump and land on ground animation
...
-- Accuracy with attachment multipliers
...
function AttemptShootGun()
	...
	local GunShootSpeedMultiplier = CurrentPlayerMultipliers["GunShootSpeedMultiplier"] -- Shooting speed
	...
	--for _,AttachmentDataTemp in pairs(Attachments) do
	...
	RECOIL_VERTICAL = BaseRecoil * tween( 0.9, 1.1, Randy:NextNumber() ) * THIRDPERSONMODIFIER -- Random value between 90% and 110% of desired recoil
	...
	-- Compute Accuracy with multipliers
	...
	if (gunCurrentStats.Melee and gunCurrentStats.Melee.Value) then
		task.spawn(function()
			--ReplicatedStorage.GunEvent["GunMeleeInitial"]:FireServer()
			...
			if ( ClosestHit ) then
				-- Override hit object just in case
		...
		BinaryUtil:WriteByte(GunFirePacket, aimTween * 255) -- Packing a double in to a byte. Doesn't need to be super precise
			...
			for i=1,pellets do
				...
				task.spawn(function()
					...
					if ( NewHitPart ) then
						...
						--BinaryUtil:WriteInt(HitPacket, #accmat)
						--for i=1,#accmat do BinaryUtil:WriteDouble(HitPacket, accmat[i]) end

						--local Packet2 = {}
						--BinaryUtil:WriteInt(Packet2, #dts)
						--for i=1,#dts do BinaryUtil:WriteDouble(Packet2, dts[i]) end

						--BinaryUtil:WriteInt(Packet2, #test_points)
						--for i=1,#test_points do
						--	BinaryUtil:WriteVector3(Packet2, test_points[i][1])
						--	BinaryUtil:WriteVector3(Packet2, test_points[i][2])
						--end
						--ReplicatedStorage.GunEvent.BulletHole:FireServer( HitPacket, HitPart, HitPart.Material, dts, test_points--[[, accmat]] )
						ReplicatedStorage.GunEvent.BulletHole:FireServer( HitPacket, NewHitPart, dts, test_points, direction--[[, accmat]] )
					end
				end)
			end
		end
...
function onEnableChange()
	if ( script.IsEnabled.Value ) then
		...
		--[[if ( script:FindFirstChild("Animate") ~= nil ) then
			getCharacter().Animate:Destroy()
			
			local newAnim = script.Animate:Clone()
			newAnim.Disabled = false
			newAnim.Parent = getCharacter()
		end--]]
...
function getFiremodeData()
	...
	-- Try to figure out which firemode we are in	
...
function handleShooting()
	...
	-- Check if our gun can shoot (has enough ammo, waited enough time, etc)
	if ( GetCanShoot() ) then

		-- Special weapons can only be fired via plugin
		local IsSpecial = isSpecial()
		if ( not IsSpecial ) then

			local FiremodeData = getFiremodeData()

			-- Find out how many bullets we need to shoot
			...
			-- If we're not reloading or reloading single shot weapon (shotgun)
			if ( not reloading or (reloading and reloadingIndividual) ) then
				-- Shoot
				AttemptShootGun()
				RepeatShoot = RepeatShoot - 1

				-- This executes on last shot of a burst
...
function releaseTrigger()
	...
	-- Reset bullets in succession (delayed by 0.1 seconds) to reset recoil
	local lastBulletId = BulletID
	task.spawn(function()
		wait(0.5)

		-- If we shot again, but too quickly, see if we can improve recoil
		-- If we are still shooting super fast, recoil wont decrease
		if ( BulletID ~= lastBulletId ) then
			...
			ShotsInSuccession = 0 -- fully reset recoil
		end
	end)
end
...
function onStep( delta )
	...
	-- Update our weapons if server changed them
	...
	if ( not crosshairGui or not crosshairGui.Parent ) then
		...
		--[[local consoleOffset = _G.IS_PLAYING_CONSOLE() and 0 or -18
		crosshairGui.Center.Position = UDim2.new( 0.5, -1, 0.5, consoleOffset )
		crosshairGui.Hitmarker.Position = crosshairGui.Center.Position]]
	end

	-- Tell server what gun we're using
	...
	-- Update weapons when you change team
	...
	if ( mobileFire ) then
		if ( canAim ) then
			...
			-- Gun cant aim, so we can instantly start firing
			holdingLeft = true
		end
	end
...
-- Test for weird bug where camera is getting stuck? Why do I even make roblox stuff :/
...
function updateCoreAnimations(force, optionalFade)
	...
	--if ( not isBaseAnimation(currentlyPlayingAnimationName) ) then
	...
	--if ( shouldRunAnimation == nil and shouldWalkAnimation == nil ) then
	--	return
	--end

	local shouldPlayAnimation = shouldRunAnimation and "Run" or (shouldWalkAnimation and "Walk" or "Idle")

	-- Load animation if it is not yet loaded
...
function updateCamera(delta)
	...
	if ( gunCurrent and gunCurrentStats and not IsDead ) then
		...
		-- Update camera offset (Sway/Recoil)
		local BOB_SCALE = tween( 0.05, 0.15, aimTween ) 
		...
		if ( not isGunDisabled ) then
			...
			local SwaySens = 1 -- TODO allow customization
		...
		-- Change FOV
		local iAimTween = getAlphaSmoothStep(aimTween, 2, 1024) --1.0 - math.pow(1.0 - aimTween,2.5)
		...
		--local ZOOM_FOV = BaseFOV * FOVMultiplier
...
function updatePlayerRotation(Delta, CameraPitch, CameraYaw)
	...
	-- Rotate Player if we're not spectating
...
function onRender1(delta)
	...
	-- Must have gun to continue
	if ( gunCurrent == nil or #gunCurrent:GetChildren() == 0 ) then
		return
	end

	-- Get zoom sensitivity multiplier from attachment
	--local tt = gunCurrent.Stats.Zoom.SensitivityMultiplier.Value	
	--local AttachmentReticle = getAttachment('AttachmentReticle')
	--if ( AttachmentReticle and AttachmentReticle.Modifiers:FindFirstChild("SensitivityMultiplier") ) then
	--	tt = tt * AttachmentReticle.Modifiers.SensitivityMultiplier.Value
	--end
	...
	-- Scope blur tweening when rotating
...
function onRender3(Delta, CameraCFrame, CameraPitch, CameraYaw)
	...
	-- Handle stepHeightOffset
	updateStepHeightOffset()

	updateCamera(Delta)

	-- Rotate Player if we're not spectating
	--updatePlayerRotation(Delta, CameraCFrame, CameraPitch, CameraYaw)
end

function onStart()
	print("Starting Gun System")

	-- Make sure guns are loaded
	...
	RunService:BindToRenderStep("GunRendering1", Enum.RenderPriority.Camera.Value-2, onRender1) -- Bulk of logic
	RunService:BindToRenderStep("GunRendering4", Enum.RenderPriority.Camera.Value-1, onRender2) -- Position Gun (first person)
	RunService.Heartbeat:Connect(onStep)
	getCustomCamera().OnPreUpdate.Event:Connect(onRender3) -- Character orientation (third person)
...
-- Ready function. Returns true when plugins are able to use the gun
...
-- CanShootGun function. Returns true if this inventory slot is able to be shot with
script.Events.CanShootGun.OnInvoke = function(inventorySlot)
	return GetCanShoot(inventorySlot)
end

-- GetCurrentGunSlot. returns the current slot index that is equipped
script.Events.GetCurrentGunSlot.OnInvoke = function()
	return gunCurrentSlot
end

-- AttemptShootGun. attempts to shoot the weapon
...
-- GetCurrentInventoryData. returns the current inventory object that is selected via gunCurrentSlot
script.Events.GetCurrentInventoryData.OnInvoke = function()
	return getInventoryItem(gunCurrentSlot)
end

-- Returns the data folder for the current gun
...
-- Returns the current playing gun animation
...
-- Returns whether the player is in third person (respects servers wishes)
...
-- Get the current speed of character
script.Events.GetSpeed.OnInvoke = function()
	return speed
end

-- Return the current aim tween
...
-- Returns whether the player is reloading
script.Events.IsReloading.OnInvoke = function()
	return reloading
end

-- Returns whether the player is aimed
script.Events.IsAimed.OnInvoke = function()
	return isAiming()--holdingRight
end

-- Play a gun sound. Gun needs to be equipped.
...
-- User event to take out weapons.
...
-- Set gun to blocked state
script.Events.SetBlocked.Event:Connect(function(Blocked)
	BlockedGun = Blocked
end)

-- Force takeout weapon if server tells us to.
...
ReplicatedStorage.GunEvent.GunForceRefresh.OnClientEvent:Connect(function()
	...
	-- Make sure gun is taken out
	if ( not gunCurrent ) then
		script.Events.TakeoutWeapon:Fire(gunCurrentSlot, true)
	end

	-- If it's STILL nil, we can't continue!
	if ( not gunCurrent ) then
		return
	end

	--gunCurrent.BulletsInMagazine.Value = InventoryItem.BulletsInMagazine.Value
	--gunCurrent.BulletsInReserve.Value = InventoryItem.BulletsInReserve.Value
end)
...
wait(1)
-- Remap console controls to PC controls
...
do
	if Player.PlayerGui:WaitForChild("MobileUI", 100000) then
		...
		UserInputService.InputEnded:Connect(function(inputObject, processed)
			...
			-- This is a really bad hack to check if we let go of thumbstick
			...
			-- If thumbstick transparency did not go up, that means we are still holding it.
			-- We do this so if we release thumbstick while shooting, we dont STOP shooting.
			mobileFire = false
			holdingLeft = false
			onClickUp(0)
		end)
